...
for d in range(7*24, (1456+1)*24):
    ...
    X[d, 0] = df.loc[df.index[(d-1):(d)], 'Price'].to_numpy() # D-1 price
    X[d, 1] = df.loc[df.index[(d-2):(d-1)], 'Price'].to_numpy() # D-2 price
    X[d, 2] = df.loc[df.index[(d-3):(d-2)], 'Price'].to_numpy() # D-3 price
    X[d, 3] = df.loc[df.index[(d-7):(d-6)], 'Price'].to_numpy() # D-7 price
    X[d, 4] = df.loc[df.index[(d):(d+1)], df.columns[1]].to_numpy() # D load forecast
    X[d, 5] = df.loc[df.index[(d-1):(d)], df.columns[1]].to_numpy() # D-1 load forecast
    X[d, 6] = df.loc[df.index[(d-7):(d-6)], df.columns[1]].to_numpy() # D-7 load forecast
    X[d, 7] = df.loc[df.index[(d):(d+1)], df.columns[2]].to_numpy() # D RES sum forecast
    X[d, 8] = df.loc[df.index[(d-1):(d)], df.columns[2]].to_numpy() # D-1 RES sum forecast
    X[d, 9] = df.loc[df.index[(d-2):(d-1)], df.columns[3]].to_numpy() # D-2 EUA
    X[d, 10] = df.loc[df.index[(d-2):(d-1)], df.columns[4]].to_numpy() # D-2 API2_Coal
    X[d, 11] = df.loc[df.index[(d-2):(d-1)], df.columns[5]].to_numpy() # D-2 TTF_Gas
    X[d, 12] = data.loc[data.index[(d-2):(d-1)], data.columns[6]].to_numpy() # D-2 Brent oil
...
if paramcount[distribution] is not None:
    ...
    #    os.path.join(f'../distparams_probNN_{distribution.lower()}', datetime.strftime(df.index[-24], '%Y-%m-%d')),
    #    'w'))
